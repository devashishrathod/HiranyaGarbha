...
export const TermsAddEdit = () => {
  ...
  const { data, isLoading: isFetching } = useGetQuery(
    isEditMode ? API_ENDPOINTS.TERMS_AND_CONDITIONS.GET_ONE.replace(":id", id) : null,
    {
      enabled: isEditMode,
  ...
  const { mutate: createItem, isPending: isCreating } = usePostMutation(
    API_ENDPOINTS.TERMS_AND_CONDITIONS.CREATE,
  );

  const { mutate: updateItem, isPending: isUpdating } = usePutMutation(
    API_ENDPOINTS.TERMS_AND_CONDITIONS.UPDATE.replace(":id", id || ""),
  );
  ...
  const handleSubmit = (e) => {
    ...
    const onSuccess = (res) => {
      toast.success(
        getApiMessage(
          res,
          isEditMode ? "Terms updated successfully" : "Terms created successfully",
        ),
      );
      setTimeout(() => navigate("/terms"), 700);
    };
