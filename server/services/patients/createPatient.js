...
/**
 * Admin flow: create the login account and the patient profile in one shot.
 * The profile merge itself is delegated to completeProfile so both flows stay in sync.
 */
exports.createPatient = async (data, image) => {
  ...
  if (email) {
    ...
    if (existing) throwError(409, "A patient with this email already exists");
  }
  ...
  if (mobile) {
    ...
    if (existing) {
      throwError(409, "A patient with this mobile number already exists");
    }
  }
  ...
  const user = await User.create({
    ...
    email,
    mobile,
    // Admin-created accounts get a throwaway password, patient resets it on first login
  ...
  const patient = await completeProfile(user._id, payload, image);
